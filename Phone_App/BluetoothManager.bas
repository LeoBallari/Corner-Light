...
Public Sub Initialize
	...
	If admin.IsEnabled = False Then
		If admin.Enable = False Then
			ToastMessageShow("Error al habilitar el adaptador Bluetooth.", True)
		Else
			ToastMessageShow("Habilitando el adaptador Bluetooth...", False)
		End If
	Else
		BluetoothState = True
	End If
...
Private Sub Serial_Connected (Success As Boolean)
	...
	If Success = False Then
		...
		'ToastMessageShow("Error al conectar: " & LastException.Message, True)
	Else
...
Private Sub AStream_NewData (Buffer() As Byte)
	...
	CallSub2(DiagnosticoActivity, "NewMessage2", BytesToString(Buffer,0, Buffer.Length, "UTF8"))
End Sub

Private Sub AStream_Error
	ToastMessageShow("La conexión está rota.....Reintentado...", True)
	Starter.Manager.ConnectTo(Main.dispo)
