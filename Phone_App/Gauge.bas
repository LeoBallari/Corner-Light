...
#End If
	...
	mlbl = Lbl
	mlbl.Font = xui.CreateFont(NativeFont, 75)
	
	mlbl.SetTextAlignment("BUTTON", "CENTER")
	mlbl.TextColor = xui.Color_Black
	mlbl.TextColor = xui.Color_White
...
Private Sub Base_Resize (Width As Double, Height As Double)
	...
	'mlbl.SetLayoutAnimated(Duration,Left,Top,Width,Height)
	'mlbl.SetLayoutAnimated(0,0,20,Width,740)
...
Private Sub DrawTicks
	...
	Dim LongTick As Int = r1 - 17dip
	Dim ShortTick As Int = r1 - 5dip
	Dim NumberOfTicks As Int
	If GaugeType = HALF_CIRCLE Then NumberOfTicks = 40 Else NumberOfTicks = 20
...
Private Sub DrawIndicator (Value As Float)
	...
	Dim angle As Float = ValueToAngle(Value) ' Ángulo en grados
	Dim p As B4XPath
	
	' Define los puntos de la aguja
	' Punto base 1 (izquierda)
	p.Initialize(x + IndicatorBaseWidth * SinD(angle), y - IndicatorBaseWidth * CosD(angle))
	
	' Punto de la punta de la aguja
	Dim tipLength As Float = Radius - 0.3 * IndicatorLength ' Ajusta este 0.3 si la aguja es muy corta
	p.LineTo(x + tipLength * CosD(angle), y + tipLength * SinD(angle))
	
	' Punto base 2 (derecha)
	p.LineTo(x - IndicatorBaseWidth * SinD(angle), y + IndicatorBaseWidth * CosD(angle))
	
	' Cierra el path para que sea una forma rellenable (un triángulo)
	p.LineTo(x + IndicatorBaseWidth * SinD(angle), y - IndicatorBaseWidth * CosD(angle)) ' Vuelve al punto inicial
	
	' Dibuja y rellena la forma de la aguja directamente
	cvsIndicator.DrawPath(p, IndicatorColor, True, 0) ' True para rellenar
	
	' --- El resto del código de la función DrawIndicator se mantiene ---
...
'	mlbl.Text = s & NumberFormat2(Value, 2, 1, 1, True) & SuffixText
	If (Value < 0) Then
		mlbl.Text = s & NumberFormat2((Value)*-1, 2, 1, 1, True) & SuffixText
	Else
		mlbl.Text = s & NumberFormat2(Value, 2, 1, 1, True) & SuffixText
	End If
End Sub
